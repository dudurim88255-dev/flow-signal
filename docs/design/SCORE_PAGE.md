...
신호 그룹은 **시장 본질에 따라 다르게 정의**. 통일하면 의미 손실:

| 시장 | 그룹 (개수) | 근거 |
|---|---|---|
| crypto | 온체인/수급, 파생/포지셔닝, 심리, 기술적 분석 (4) | 온체인 + 파생 데이터 풍부 |
| korea | 수급 흐름, 리스크 지표, 기술적 분석 (3) | 외국인/기관 매매 + 신용/공매도 리스크 중심 |
| us | 수급+모멘텀, 기술적 분석, 시장 맥락 (3) | SPY 상대강도 + 변동성 등 거시 비중 |

**한국 "수급 흐름" 그룹의 K8 (외국계 창구) 포함 의도**: 시그널 ID 순서가 K1~K4, K8 로 비연속이지만, K8 (외국계 창구) 의 의미가 **외국인 매매 행태** 분석이라 "수급" 카테고리. K5 (공매도), K6 (대차잔고), K7 (신용잔고) 는 **차입성 리스크** 카테고리. 따라서:

- 수급: K1, K2, K3, K4, K8 (외국인/기관/프로그램/외국인 보유율/외국계 창구)
- 리스크: K5, K6, K7 (공매도/대차/신용)
- 기술: K9, K10, K11, K12

이 분류는 영구 — 변경 시 흥권 검토 + 백테스트 결과 재검증 필수.
